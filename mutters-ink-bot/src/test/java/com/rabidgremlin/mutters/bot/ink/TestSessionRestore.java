...
/**
 * Test session restores when story changes (which can happen if sessions are
 * stored outside of the bot for instance in a external cache).
 * 
 * Ink tries to gracefully handle story state restores when a story structure
 * has changed. It is supposed to raise warnings and errors that can be checked
 * when this occurs, but this is not happening. There is also a case where
 * restore causes wrong path to be followed and incorrect output to be
 * generated. See testSessionRestoreWhenStoryOptionsChange()
 * 
 * Current 'hack' is treat any change of story as an incompatible state change
